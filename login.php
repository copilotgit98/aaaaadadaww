...
if(isset($_POST["cadastro"])) {
    ...
    if($check->num_rows > 0 || $checkAdm->num_rows > 0) {
        ...
        $conn->query("INSERT INTO usuarios (nome, email, senha, tipo) VALUES ('$nome', '$email', '$senha', 0)");
...
if(isset($_POST["login"])) {
  ...
  // Verifica se o e-mail existe em usuarios
  ...
  if($res_check->num_rows) {
    ...
    // Mesma lógica de admin
    $sql_adm = "SELECT id, email, nome, senha, tipo FROM adm WHERE email='$email' AND senha='$senha' LIMIT 1";
    $res_adm = $conn->query($sql_adm);
    if($res_adm->num_rows) {
      $_SESSION["usuario"] = $res_adm->fetch_assoc();
